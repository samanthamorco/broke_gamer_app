    ...
    systems.each do |system, id|
      if params[:system] == system
        if params[:page]
          ...
          hash = Unirest.get("http://api.bestbuy.com/v1/products(categoryPath.id=#{id})?show=sku,image,name,shortDescription,productID,salePrice,onSale&pageSize=12&page=#{page}&format=json&apiKey=5rnsy6rxdnw33npjhqnj4m4q").body
